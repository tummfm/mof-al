...
def uq_calibration(uq_samples, targets, mask=None, verbose=False):
    ...
    Args:
        ...
        verbose: Print additional calibration statistics
    """
    ...
    if verbose:
        absolute_errors = jnp.abs(predicted_mean - targets)
        average_abs_deviation = jnp.mean(
            jnp.abs(uq_samples - predicted_mean[:, jnp.newaxis, :]), axis=1)

        if mask is not None:
            absolute_errors = absolute_errors[mask]
            average_abs_deviation = average_abs_deviation[mask]

        mean_cal_var = jnp.mean(alpha_sq * predicted_var)
        mean_cal_ave_dev = jnp.mean(alpha * average_abs_deviation)
        print(f'Before calibration: MSE = {jnp.mean(squared_errors)},'
              f' Mean Variance = {jnp.mean(predicted_var)}')
        print(f'After calibration: Mean Variance = {mean_cal_var}')
        print(f'Before calibration: MAE = {jnp.mean(absolute_errors)}, Mean '
              f'Average absolute deviation = {jnp.mean(average_abs_deviation)}')
        print(f'After calibration: Average deviation = {mean_cal_ave_dev}')
    return alpha
