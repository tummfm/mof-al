...
def sparse_graph_from_positions(r_cutoff, positions, box, species=None):
    """Constructs the SparseDirectionalGraph from partcle positions and box.

    Note that this function is not jittable.

    Args:
        r_cutoff: Radial cut-off distance below which 2 particles form an edge.
        positions: A (N_particles, dim) array of particle positions.
        box: A single 1 or 2-dimensional box.
        species: A (N_particles,) array of particle species.

    Returns:
        SparseDirectionalGraph representation of the particles in the box.
        Capped exactly.
    """
...
def convert_dataset_to_graphs(r_cutoff, position_data, box, species,
    ...
    for (positions, cur_box) in zip(position_data, box):
        graph = sparse_graph_from_positions(r_cutoff, positions, cur_box)
